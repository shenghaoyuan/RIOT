...
void print_jit_state(struct jit_state* st){
  print_u64_hex(*((*st).jit_ins + (*st).pc_loc));
  printf("(PC)\n");
  printf("pc= %02d flag= %d\n",  (*st).pc_loc, (*st).flag);
    print_u32_dec((*st).regs_st[0]);
    printf("(R0)\n");
    print_u32_dec((*st).regs_st[1]);
    printf("(R1)\n");
    print_u32_dec((*st).regs_st[2]);
    printf("(R2)\n");
    print_u32_dec((*st).regs_st[3]);
    printf("(R3)\n");
    print_u32_dec((*st).regs_st[4]);
    printf("(R4)\n");
    print_u32_dec((*st).regs_st[5]);
    printf("(R5)\n");
    print_u32_dec((*st).regs_st[6]);
    printf("(R6)\n");
    print_u32_dec((*st).regs_st[7]);
    printf("(R7)\n");
    print_u32_dec((*st).regs_st[8]);
    printf("(R8)\n");
    print_u32_dec((*st).regs_st[9]);
    printf("(R9)\n");
    print_u32_dec((*st).regs_st[10]);
    printf("(R10)\n");
  return ;
}

/*
void print_entry_point(struct jit_state* st){
  printf("entry_point list: ");
  for(unsigned i = 0; i < (*st).entry_len; i++) {
    printf("%d; ", (*st).ep_list[i]);
  }
  printf("\n");
  return ;
}

void print_arm32(struct jit_state* st){
  printf("arm32 list: ");
  for(unsigned i = 0; i < (*st).arm32_len; i++) {
    printf("%x: %u; ", i, (*st).arm32[i]);
    if ((i+1)%8 == 0) { printf("\n"); }
  }
  printf("\n");
  return ;
...
void print_ibpf(struct jit_state* st){
  ...
  for(unsigned i = 0; i < (*st).ins_len; i++) {
    print_bpf_insstruction((*st).ibpf[i]);
    printf("(%d)\n", i);
  }
  printf("\n");
  return ;
