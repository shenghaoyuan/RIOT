...
static __attribute__((always_inline)) inline _Bool check_pc (struct bpf_state* st) {
  return (*st).state_pc < (*st).ins_len;
}
static __attribute__((always_inline)) inline _Bool check_pc_incr(struct bpf_state* st) {
  return (*st).state_pc+1 < (*st).ins_len;
}
...
static __attribute__((always_inline)) inline unsigned long long eval_ins(struct bpf_state* st)
{
  return *((*st).ins + (*st).state_pc);
}

static __attribute__((always_inline)) inline unsigned long long eval_cache(struct bpf_state* st)
{
  return *((*st).cache + (*st).state_pc);
}

static __attribute__((always_inline)) inline void upd_cache(struct bpf_state* st, unsigned int v)
{
  (*st).cache[(*st).state_pc] = v;
  return ;
}
...
unsigned char *check_mem_aux(struct bpf_state* st, unsigned int num, unsigned int cache_id, unsigned int perm, unsigned int chunk, unsigned int addr, struct memory_region *mrs)
{
  unsigned int n;
  ...
  if (num == 0U) {
    ...
    if (num == cache_id) {
      return check_mem_aux(st, n, cache_id, perm, chunk, addr, mrs);
    } else {
...
static __attribute__((always_inline)) inline unsigned char *check_mem_normal(struct bpf_state* st, unsigned int perm, unsigned int chunk, unsigned int addr, unsigned int cache_id)
{
  ...
  check_ptr =
    check_mem_aux(st, mem_reg_num, cache_id, perm, chunk, addr,
                  mrs);
  is_null = cmp_ptr32_nullM(check_ptr);
  if (is_null) {
    return 0;
...
static __attribute__((always_inline)) inline unsigned char *check_mem(struct bpf_state* st, unsigned int perm, unsigned int chunk, unsigned int addr)
{
  ...
  unsigned char *check_ptr;
  _Bool is_null;
  cache_id = eval_cache(st);
  if (0U == cache_id) {
    return check_mem_normal(st, perm, chunk, addr, cache_id);
  } else {
    mrs = eval_mrs_regions(st);
    cur_mr = get_mem_region(cache_id - 1U, mrs);
    check_ptr =
      check_mem_aux2(cur_mr, perm, addr, chunk);
    is_null = cmp_ptr32_nullM(check_ptr);
    if (is_null) {
      return check_mem_normal(st, perm, chunk, addr, cache_id);
    } else {
...
static __attribute__((always_inline)) inline void step_opcode_branch(struct bpf_state* st, unsigned long long dst64, unsigned long long src64, unsigned int ofs, unsigned char op)
{
...
static __attribute__((always_inline)) inline void step(struct bpf_state* st)
{
  ...
  switch (opc) {
    ...
    case 5:
      ...
      ofs = get_offset(ins);
      src64 = get_src64(st, op, ins);
      step_opcode_branch(st, dst64, src64, (unsigned int) ofs,
                         op);
      return;
    case 0:
      dst64 = eval_reg(st, dst);
...
unsigned long long bpf_interpreter(struct bpf_state* st, unsigned int fuel, unsigned int ctx_ptr)
{
  unsigned int f;
  unsigned long long res;
  upd_reg(st, 1U, (unsigned long long) ctx_ptr);
