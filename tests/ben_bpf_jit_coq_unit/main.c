...
#else
        bpf_t bpf = {
            .application = (uint8_t*)&test_app.text,
            .application_len = sizeof(test_app.text),
            .stack = _bpf_stack,
...
#ifdef MODULE_GEN_BPF
        int result = bpf_interpreter(&st, 10000, 0);
#elif defined(MODULE_GEN_IBPF)
        int result = ibpf_interpreter(&ibpf_state.st, 10000, 0);
#else
